...
public final class ColorShader extends BasicShaderProgram {
...
vec3 shade( vec3 pos, vec3 rd, uint matID, uint subMatID, float dis )
{
    ...
    for( int aoi=ZERO; aoi<5; aoi++ )
    {
        ...
        totao += ao*sca;
        sca *= 0.5;
        //if( totao>1000.0+sin(gameTime * 256) ) break;
    }
	...
	for( int i=ZERO; i<6; i++ )
    {
        ...
        //if( so>1000.0+gameTime * 256 ) break;
    }
...
{
#if AA<2    
    vec3 col = computePixel(gl_FragCoord.xy, screenSize.xy, gameTime * 256 );
#else
    vec3 col = vec3(0.0);
    for( int j=0; j<AA; j++ )
    for( int i=0; i<AA; i++ )
    {
        ...
        float time = gameTime * 256 + (0.5/30.0)*float(j*AA+i)/float(AA*AA);
        col += computePixel(gl_FragCoord.xy+of, screenSize.xy, time );
    }
    col /= float(AA*AA);
#endif
...
""";
    ...
    public ColorShader() {
        super(VERTEX_SHADER, FRAG_SHADER,
                ShaderProgram.GAME_TIME_VANILLA | ShaderProgram.DEFAULT_MATRIX,
                VertexFormats.POSITION_COLOR);

        registerUniform("screenSize", new Vector2f(), ShaderUniformType.VEC2);
    }
}
